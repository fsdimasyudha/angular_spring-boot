...
import com.dimasyudha.dimasyudha.config.KafkaSseConsumer;
import com.dimasyudha.dimasyudha.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/dimasyudha-kafka")
public class DimasyudhaKafkaResource {

    private final Logger log = LoggerFactory.getLogger(DimasyudhaKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public DimasyudhaKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
